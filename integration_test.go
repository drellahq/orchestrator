...
// and that the terraform-libvirt provider can find its socket.
//
// On Fedora's modular libvirt the session daemon (virtqemud) auto-starts
// via socket activation but exposes virtqemud-sock, not libvirt-sock.
// The terraform-libvirt provider only looks for libvirt-sock, so we
// create a symlink.
//
// On Ubuntu's monolithic libvirtd the session daemon already creates
// libvirt-sock directly, so no symlink is needed.
func ensureSessionSocket(t *testing.T) {
	...
	// If libvirt-sock already exists (Ubuntu monolithic libvirtd), nothing to do.
	if _, err := os.Stat(legacySock); err == nil {
		return
	}
	// Fedora modular: symlink libvirt-sock → virtqemud-sock
	target := filepath.Join(sockDir, "virtqemud-sock")
	if _, err := os.Stat(target); err != nil {
		t.Fatalf("neither libvirt-sock nor virtqemud-sock found in %s", sockDir)
	}
