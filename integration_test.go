...
func TestIntegration(t *testing.T) {
	...
	mcpSrv := mcpserver.New(logger, sandboxName, taskDir, runner, prOpener, nil, []string{"test/*"}, "", "")
...
func TestIntegrationWithAuthor(t *testing.T) {
	...
	prOpener := &testPROpener{}
	logger := slog.New(slog.NewTextHandler(os.Stderr, &slog.HandlerOptions{Level: slog.LevelDebug}))
	mcpSrv := mcpserver.New(logger, authorSandboxName, taskDir, runner, prOpener, nil, []string{"test/*"}, "Test User <test@example.com>", "")
	if err := mcpSrv.Start(); err != nil {
		t.Fatalf("MCP server start: %v", err)
	}
	defer func() { _ = mcpSrv.Stop(ctx) }()
...
echo "Result: $RESULT"`, remotePort, remotePort, remotePort)
	...
	mcpSrv2 := mcpserver.New(logger, authorSandboxName, taskDir, runner, prOpener2, nil, []string{"test/*"}, state.Author, "")
