...
func TestCheckForNewIssues_ForwardsLabels(t *testing.T) {
	if _, err := exec.LookPath("sh"); err != nil {
		t.Skip("sh not found")
	}
	...
	rawJSON := `[{"number":7,"title":"Build RHEL image","body":"Build it","user":{"login":"alice"},"labels":[{"name":"rhel"},{"name":"priority"}]}]`
	script := writeIssuesScript(t, rawJSON)
	...
	if capturedLabels[0] != "rhel" || capturedLabels[1] != "priority" {
		t.Errorf("labels = %v, want [rhel priority]", capturedLabels)
	}
}
